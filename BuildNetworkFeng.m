...
seed= 12345788; %(random1 12345789) % control the sequencies of generation the random values.
%seed= 3344;%(random2)
...
dmin = 0.015;   % minimum distance between soma.


upscale=27;
PCELL=820*upscale; %%840

INTCELL=180*upscale;   %%according to Drew, only 18% is FS.
NCELL=PCELL+INTCELL;

TypeA=590*upscale;%Anone=110*upscale;ADA=190*upscale;ANE=120*upscale; ADANE=220*upscale;    %%70,110,79,141  %%%%%according to Drew data, A took up ~65% of Pyr
TypeB=0;%TypeB=240; Bnone=40;BDA=65;BNE=48; BDANE=87;
TypeC=230*upscale; %Cnone=57*upscale;CDA=64*upscale;CNE=50*upscale; CDANE=89*upscale;    %%27,44,32,57
...
n=0;
m=0;
xI=(dmin*2:dmin*2:AP_I)';
yI=(dmin*2:dmin*2:LAT_I)';
zI=(dmin*2:dmin*2:DV_I)';
loca=[];loc=[];
for z=dmin:dmin:DV;
...
clear dis;
conn_c_pre=cell(NCELL,1);  %%%each row stands for pre
conn_c_post=cell(NCELL,1);  %%%each row stands for post

%%%define connectivity coefficient
% Ap2p=0.04691;Bp2p=-0.004382;
% Ap2i=0.4628;Bp2i=-0.004398;
% Ai2p=0.2187;Bi2p=-0.004228;

% Ap2p=0.02427;Bp2p=-0.003068;
% Ap2i=0.3688;Bp2i=-0.005105;
% Ai2p=0.16435;Bi2p=-0.00539;
...
%%%%for PN->PN, PN->ITN, ITN->PN connections%%%%%%
for pre=1:NCELL;  %%%%pre cell
    for post=1:NCELL;  %%%%post cell
        if pre~=post;
       if pre<=PCELL&&post<=PCELL;  %%%P2P
           prob=Ap2p*exp(1000*dis_matrix(pre,post)*Bp2p);
           if rand<prob
               conn_c_pre{pre,1}=[conn_c_pre{pre,1},post];
               conn_c_post{post,1}=[conn_c_post{post,1},pre];
           end
       elseif pre<=PCELL&&post>PCELL;  %%%P2I
           prob=Ap2i*exp(1000*dis_matrix(pre,post)*Bp2i);
            if rand<prob
               conn_c_pre{pre,1}=[conn_c_pre{pre,1},post];
               conn_c_post{post,1}=[conn_c_post{post,1},pre];
            end
       elseif pre>PCELL&&post<=PCELL;  %%%I2P
           prob=Ai2p*exp(1000*dis_matrix(pre,post)*Bi2p);
            if rand<prob
               conn_c_pre{pre,1}=[conn_c_pre{pre,1},post];
               conn_c_post{post,1}=[conn_c_post{post,1},pre];
            end
...
slice_dimension=0.3*1;%%%%%slice dimension is defined according to Wooduff 2007
%%%%for I-P pair%%%%%%%
...
%%%calculate each cell's nearby cells within region
for i=1:INTCELL
num_IP_300(i,1)=numel(find(IP_dis_300(i,:)>0));
end
...
%%%calculate each cell's nearby cells within region
for i=1:PCELL
num_PI_300(i,1)=numel(find(PI_dis_300(i,:)>0));
end
...
%%%calculate each cell's nearby cells within region
for i=1:INTCELL
num_II_300(i,1)=numel(find(II_dis_300(i,:)>0));
end
...
%  %%%within 600um     
% P2P_slice_offset=0.00;
% [a_600,b_600]=find((dis_matrix(1:PCELL,1:PCELL)-(0.6+P2P_slice_offset))<=0.0001&dis_matrix(1:PCELL,1:PCELL)>(0.5+P2P_slice_offset));%%%within 500um
...
%     
% 
% %%%%calculate nearby numbers within region
% for i=1:PCELL
% num_PP_50(i,1)=numel(find(PP_dis_50(i,:)>0));
% num_PP_100(i,1)=numel(find(PP_dis_100(i,:)>0));
% num_PP_200(i,1)=numel(find(PP_dis_200(i,:)>0));
% num_PP_300(i,1)=numel(find(PP_dis_300(i,:)>0));
% num_PP_400(i,1)=numel(find(PP_dis_400(i,:)>0));
% num_PP_500(i,1)=numel(find(PP_dis_500(i,:)>0));
% num_PP_600(i,1)=numel(find(PP_dis_600(i,:)>0));
% 
% end
%     
% %%%%make connection%%%%%%
...
% P2P_conn_50=0.03*P2P_conn_scale_local;  %%%connectivity within 50um
% P2P_conn_100=0.02*P2P_conn_scale_local;  %%%connectivity within 50-100um
% P2P_conn_200=0.01*P2P_conn_scale_local;  %%%connectivity within 100-200um
% P2P_conn_300=0.005*P2P_conn_scale;  %%%connectivity within 200-300um
% P2P_conn_400=0.005*P2P_conn_scale;  %%%connectivity within 300-400um
% P2P_conn_500=0.005*P2P_conn_scale;  %%%connectivity within 400-500um
% P2P_conn_600=0.005*P2P_conn_scale;  %%%connectivity within 500-600um
% 
% 
% Albany
...
%%%%generate rotated oritantion for 2 dends%%%%%%

% N=10000;                       %%%method is from website http://mathworld.wolfram.com/SpherePointPicking.html%%%%
% X=randn(N,1);
% X2=X.^2;
% Y=randn(N,1);
% Y2=Y.^2;
...
% 
% oritation = randompo(randperm(N,NCELL),:);  %%%record oritation for each cell
% figure
% scatter3(oritation(:,1),oritation(:,2),oritation(:,3),'.');
% 
% %%% Partly rotated.
% max_deg = 30;   % Limit angles away from pole.
% theta = 2*pi*rand(NCELL,1);

% %%% Not evenly distributed. Denser at pole.
% phi = max_deg/180*pi*rand(NCELL,1);
% Xp = cos(theta).*sin(phi);
% Yp = sin(theta).*sin(phi);
% Zp = cos(phi);
% part_rot = [Xp,Yp,Zp];
% hold on
% scatter3(part_rot(:,1),part_rot(:,2),part_rot(:,3),'r','.');

%%% Evenly distributed on sphere.
% Zp = unifrnd(cos(max_deg/180*pi),1,NCELL,1);
% rXY = sqrt(1-Zp.*Zp);
% Xp = rXY.*cos(theta);
% Yp = rXY.*sin(theta);
% part_rot = [Xp,Yp,Zp];
...
% 
% %%% No rotation.
% nooritation = repmat([0 0 1],NCELL,1);  %%%record oritation for each cell
%%%%%%%oritation%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 
% for i=1:size(active_syn_GAP,1)
%     temp=[];temp=active_syn_GAP(i,:);
%     temp(temp<=0)=[];
...
for i=1:PCELL;
    if numel(conn_c_post{i,1})>0;
  clear fmt; fmt=[repmat('%d\t',1,numel(conn_c_post{i,1})) '\n'];   %%%%format
  fprintf(fid,fmt,conn_c_post{i,1}-1);   %%%transfer to neuron ID at the same time%%%%%
    end
prenum(i,1)=numel(conn_c_post{i,1});
end

for i=1:INTCELL;
  clear row_I2I; clear col; [row_I2I,col]=find(active_syn_I2I==i+PCELL-1); row_I2I=row_I2I+PCELL;%%%row represents pre ID
  clear row_com; row_com=[conn_c_post{i+PCELL,1}-1,row_I2I'-1];%%%transfer to neuron ID at the same time%%%%%
  if numel(row_com)>0;
      clear fmt; fmt=[repmat('%d\t',1,numel(row_com)) '\n'];   %%%%format
            fprintf(fid,fmt,row_com);   %%%transfer to neuron ID at the same time%%%%%
            end
  prenum(i+PCELL,1)=numel(row_com); 
end
